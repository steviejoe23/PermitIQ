...
if st.session_state.parcel_data:
    ...
    if "geometry" in data:
        try:
            ...
            raw_coords = data["geometry"].get("coordinates")
            if not raw_coords:
...
if st.session_state.prediction_result:
        ...
        with st.expander("Model Explainability — What drove this prediction?", expanded=False):
            ...
            for d in top_drivers:
                ...
                st.markdown(
                    ...
                    f'<span style="width:260px;font-size:14px;">{esc(d.get("feature", "Unknown"))}</span>'
    ...
    if api_recs:
        ...
        if len(result.get("variances", [])) > 3:
            recommendations.append(f"Reduce scope — you're requesting {len(result.get('variances', []))} variances; fewer = better odds")
        if prob >= 0.6:
        ...
        if compare_res and compare_res.status_code == 200:
            ...
            if scenarios:
                ...
                for s in scenarios:
                    ...
                    whatif_lines.append(f"- {s.get('scenario', 'Unknown')}: {s.get('probability', 0):.0%} ({diff:+.1%})")
...
"""
    def _format_timeline_html(timeline):
        ...
        med = timeline.get('median_days', 0)
        p25 = timeline.get('p25_days', med)
        p75 = timeline.get('p75_days', med)
        ward_str = f" (Ward {timeline.get('ward', '')})" if timeline.get('ward_specific') else ""
...
st.markdown(
    '<div style="margin:48px 0 8px 0;padding:16px 0 12px 0;border-top:2px solid #334155;">'
    '<div style="display:flex;align-items:center;gap:12px;">'
    '<span style="font-size:24px;font-weight:800;color:#f1f5f9;letter-spacing:-0.5px;">Intelligence Reports</span>'
    '<span style="background:linear-gradient(135deg,#3b82f6,#8b5cf6);color:#fff;'
    'font-size:11px;font-weight:700;padding:3px 10px;border-radius:12px;letter-spacing:0.5px;">PREMIUM</span>'
    '</div>'
    '<p style="color:#94a3b8;font-size:13px;margin:4px 0 0 0;">'
    'Actionable intelligence for your ZBA hearing. Tactical advice, risk scoring, and strategic timing.</p>'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    '<div style="margin:48px 0 8px 0;padding:16px 0 12px 0;border-top:2px solid #334155;">'
    '<span style="font-size:24px;font-weight:800;color:#f1f5f9;letter-spacing:-0.5px;">Research Tools</span>'
    '<p style="color:#94a3b8;font-size:13px;margin:4px 0 0 0;">'
    'Deep-dive into ward data, market trends, attorney profiles, and zoning districts.</p>'
    '</div>',
    unsafe_allow_html=True
)
                ...
                try:
                    ...
                    if tl_res.status_code == 200:
                        ...
                        if ward_tl and "phases" in ward_tl:
                            ...
                            if ftd:
                                st.caption(f"Median decision timeline for Ward {ward_input}: {ftd.get('median_days', 'N/A')} days ({ftd.get('cases_used', 0)} cases)")
                except Exception:
                    pass
            ...
            if atty_search_res.status_code == 200:
                ...
                if atty_results:
                    ...
                    selected_attorney = st.selectbox(
                        ...
                        options=[a.get("name", "Unknown") for a in atty_results],
                        format_func=lambda n: next(
                            (f"{a.get('name', 'Unknown')} — {a.get('approval_rate', 0):.0%} win rate ({a.get('total_cases', 0)} cases)"
                             for a in atty_results if a.get('name') == n), n
                        ),
                        key="atty_select"
                    )
                        ...
                        try:
                            ...
                            if profile_res.status_code == 200:
                                ...
                                if streak.get("length", 0) >= 3:
                                    streak_emoji = "W" if streak.get("type") == "APPROVED" else "L"
                                    st.markdown(f"Current streak: **{streak.get('length', 0)} {streak_emoji}**")
...
with st.expander("Case Number Lookup", expanded=False):
    st.markdown(
